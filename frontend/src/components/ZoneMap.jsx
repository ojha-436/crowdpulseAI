...
const riskColors = {
  low: { bg: 'bg-pulse-400/15', border: 'border-pulse-400/20', text: 'text-pulse-400', fill: 'bg-pulse-400' },
  medium: { bg: 'bg-cyan-400/15', border: 'border-cyan-400/20', text: 'text-cyan-400', fill: 'bg-cyan-400' },
  high: { bg: 'bg-warn-400/15', border: 'border-warn-400/20', text: 'text-warn-400', fill: 'bg-warn-400' },
  critical: { bg: 'bg-alert-400/15', border: 'border-alert-400/20', text: 'text-alert-400', fill: 'bg-alert-400' },
};
        ...
        <div className="flex items-center gap-2">
          {['low', 'medium', 'high', 'critical'].map((r) => (
            <div key={r} className="flex items-center gap-1">
              ...
              <span className="text-[9px] text-gray-500 uppercase">{r}</span>
            </div>
          ))}
        </div>
      </div>

      <div className={`grid gap-2 ${expanded ? 'grid-cols-2 md:grid-cols-3 lg:grid-cols-4' : 'grid-cols-2'}`}>
              ...
              <div className="flex items-center gap-3 mb-2">
                <div className="relative w-12 h-12">
                  <svg className="w-12 h-12 -rotate-90" viewBox="0 0 48 48">
                    ...
                    <circle
                      ...
                      stroke={zone.riskLevel === 'critical' ? '#ff4757' : zone.riskLevel === 'high' ? '#ff8c00' : zone.riskLevel === 'medium' ? '#22d3ee' : '#38f2b0'}
                      strokeWidth="4"
                  ...
                  <span className={`absolute inset-0 flex items-center justify-center text-xs font-bold font-mono ${rc.text}`}>
                    {densityPct}%
                  </span>
                </div>
                <div className="flex-1 min-w-0">
                  <div className="text-[11px] text-gray-400">
                    ...
                    <span className="text-gray-600"> / {zone.capacity.toLocaleString()}</span>
                  </div>
