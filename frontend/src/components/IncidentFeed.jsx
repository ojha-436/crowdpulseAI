...
export default function IncidentFeed({ incidents, expanded }) {
  ...
  return (
    <div className="glass-card p-5">
      ...
      <div
        className={`space-y-2 ${expanded ? "max-h-[70vh]" : "max-h-[320px]"} overflow-y-auto pr-1`}
      >
        {list.length === 0 && (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-[10px] text-gray-400 ml-auto shrink-0">{timeStr}</span>
                </div>
                <p className="text-xs text-gray-300 leading-relaxed">{incident.description}</p>
                <p className="text-[10px] text-gray-400 mt-1">Zone: {incident.zone}</p>
              </div>
              {isActive && (
                <button
