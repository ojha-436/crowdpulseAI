...
export default function MetricCards({ state, analysis }) {
  ...
  const avgQueue =
    Object.values(state.gates).reduce((sum, g) => sum + g.queueLength, 0) / totalGates;
  ...
  const cards = [
      ...
      color:
        occupancyPct > 85
          ...
          : occupancyPct > 60
            ...
            : "text-pulse-400",
      barColor:
        occupancyPct > 85 ? "bg-alert-400" : occupancyPct > 60 ? "bg-warn-400" : "bg-pulse-400",
    },
    {
      label: "Active Gates",
      value: `${openGates}`,
    ...
    {
      label: "Flow Rate",
      value: Object.values(state.gates)
        .reduce((s, g) => s + g.currentFlow, 0)
        .toLocaleString(),
