 ...
 *   - language {string} [optional] - BCP-47-style code (e.g. "es", "fr") selecting
 *     the response language; unsupported/omitted values fall back to English.
...
router.post("/api/agent/query", authMiddleware, async (req, res) => {
  ...
  // Resolve the requested response language, defaulting to English when the
  // code is missing or unsupported. Only the human-readable name reaches Gemini.
  const languageName =
  ...
  try {
    const systemPrompt = buildSystemPrompt(stadiumState, languageName);
